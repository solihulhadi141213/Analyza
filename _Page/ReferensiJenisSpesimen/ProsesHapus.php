<?php
    ...
    if (empty($id_referensi_jenis_spesimen)) {
    ...
    $QryCheck = $Conn->prepare("SELECT id_referensi_jenis_spesimen FROM referensi_jenis_spesimen WHERE id_referensi_jenis_spesimen = ?");
    if (!$QryCheck) {
    ...
    $QryCheck->bind_param("i", $id_referensi_jenis_spesimen);
    $QryCheck->execute();
    ...
    try {
        ...
        $QryDelete = $Conn->prepare("DELETE FROM referensi_jenis_spesimen WHERE id_referensi_jenis_spesimen = ?");
        if (!$QryDelete) {
            throw new Exception($Conn->error);
        }
        $QryDelete->bind_param("i", $id_referensi_jenis_spesimen);
