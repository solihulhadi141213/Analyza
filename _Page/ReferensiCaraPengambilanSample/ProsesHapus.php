<?php
    ...
    if (empty($id_referensi_metode_sample)) {
    ...
    $QryCheck = $Conn->prepare("SELECT id_referensi_metode_sample FROM referensi_metode_sample WHERE id_referensi_metode_sample = ?");
    if (!$QryCheck) {
    ...
    $QryCheck->bind_param("i", $id_referensi_metode_sample);
    $QryCheck->execute();
    ...
    try {
        ...
        $QryDelete = $Conn->prepare("DELETE FROM referensi_metode_sample WHERE id_referensi_metode_sample = ?");
        if (!$QryDelete) {
            throw new Exception($Conn->error);
        }
        $QryDelete->bind_param("i", $id_referensi_metode_sample);
