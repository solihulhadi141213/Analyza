<?php
    ...
    if(empty($_POST['id_laboratorium_procedure'])){
        echo json_encode(['status'  => 'error','message' => 'ID Lokal Procedure Tidak Boleh Kosong!']);
        exit;
    }
    ...
    if($update_procedure==1){
        ...
        if(empty($id_procedure)){
            echo json_encode(['status'  => 'error','message' => 'Sebelumnya belum mengirim Resource <i>Procedure</i> ke SATUSEHAT!']);
            exit;
        }
        if(empty($ihs_pasien)){
    ...
    // Update Ke Database Procedure
    $query = $Conn->prepare("
        UPDATE laboratorium_procedure SET
            ...
            datetime_start = ?,
            datetim_end = ?
        WHERE id_laboratorium_procedure = ?
    ");
    ...
    if (!$query->execute()) {
        echo json_encode([
            'status'  => 'error',
            'message' => 'Gagal memperbarui data procedure'
        ]);
    } 
    $query->close();
