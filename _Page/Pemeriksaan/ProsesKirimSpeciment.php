<?php
    ...
    if(empty($_POST['id_laboratorium_spesimen'])){
        echo json_encode(['status'  => 'error','message' => 'ID sPESIMEN Laboratorium Tidak Boleh Kosong!']);
        exit;
    }
    ...
    $id_laboratorium_spesimen = $_POST['id_laboratorium_spesimen'];
   ...
   // GENERATE TOKEN SATUSEHAT
    ...
    $url_api = $url_api . '/fhir-r4/v1/Specimen';
   ...
   // DECODE RESPONSE
    ...
    if ($http_code !== 201) {
        $msg = 'Gagal mengirim Spesimen ke SATUSEHAT';
    ...
    $id_speciment = $result['id'] ?? null;

     if ($id_speciment) {
        $upd = $Conn->prepare("UPDATE laboratorium_spesimen SET id_speciment = ? WHERE id_laboratorium_spesimen = ?");
        $upd->bind_param("si", $id_speciment, $id_laboratorium_spesimen);
        $upd->execute();
        $upd->close();
    }
