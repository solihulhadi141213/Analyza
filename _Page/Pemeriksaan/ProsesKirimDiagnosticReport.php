<?php
   ...
   // GENERATE TOKEN SATUSEHAT
    ...
    if(empty($_POST['id_diagnostic_report'])){
        $url_api = $url_api . '/fhir-r4/v1/DiagnosticReport';
        $metode = "POST";
    }else{
        $id_diagnostic_report = $_POST['id_diagnostic_report'];
        $url_api = $url_api . '/fhir-r4/v1/DiagnosticReport/'.$id_diagnostic_report.'';
        $metode = "PUT";
    }
   ...
   // DECODE RESPONSE
    ...
    $id_diagnostic_report = $result['id'] ?? null;
     ...
     if ($id_diagnostic_report) {
        $upd = $Conn->prepare("UPDATE laboratorium_rincian SET id_diagnostic_report = ? WHERE id_laboratorium_rincian = ?");
        $upd->bind_param("si", $id_diagnostic_report, $id_laboratorium_rincian);
        $upd->execute();
        $upd->close();
    }
