<?php
    ...
    if(empty($_POST['id_referensi_range'])){
        echo json_encode(['status'  => 'error','message' => 'ID Nilai Rujukan Tidak Boleh Kosong!']);
        exit;
    }
    ...
    // Buat Variabel
    $id_referensi_range       = $_POST['id_referensi_range'];
    ...
    if ($query->execute()) {
        echo json_encode([
            'status'  => 'success',
            'message' => 'Data referensi range pemeriksaan berhasil diperbarui'
        ]);
    } else {
        echo json_encode([
            'status'  => 'error',
